...
//Importamos el modulo de pubsub
const pubsub = require('./pubsub'); // Importa el módulo pubsub.js
...
const resolvers = {
  ...
  Mutation: {
    ...
    createTask: async (_, args, { pubsub }) => {
      const newTask = new Task(args);
      await newTask.save();

      // Publica el evento "taskCreated" con la nueva tarea
...
//Importamos el modulo pubsub al archivo principal del servidor apollo
const pubsub = require('./pubsub'); // Importa el módulo pubsub.js
...
const server = new ApolloServer({
  ...
  resolvers,
  context: ({ req, res }) => ({
    pubsub, // Agrega pubsub al contexto
    req,
    res,
  }),
});
